...
bool8             S9xMixSamples(uint8 *dest, int sample_count)
{
        ...
        if (spc::resampler->avail() >= sample_count) {
            ...
            if (Settings.MSU1) {
                if (msu::resampler->avail() >= sample_count) {
                    ...
                    msu::resampler->read(msu::resample_buffer, sample_count);
    ...
    if (spc::resampler->space_empty() >= 535 * 2 || !Settings.SoundSync || Settings.TurboMode || Settings.Mute)
...
{
    spc::dynamic_rate_multiplier =
        1.0 + (Settings.DynamicRateLimit * (buffer_size - 2 * avail)) / (double)(1000 * buffer_size);
    UpdatePlaybackRate();
}
static void UpdatePlaybackRate(void)
{
...
{
    SNES::smp.clock -= S9xAPUGetClock(g_snes->scpu->Cycles);
    SNES::smp.enter();
...
void S9xAPULoadBlarggState(uint8 *oldblock)
{
    ...
    int cur_time = S9xAPUGetClock(g_snes->scpu->Cycles);
