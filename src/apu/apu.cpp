...
static uint8 APUROM[64] = {0xCD, 0xEF, 0xBD, 0xE8, 0x00, 0xC6, 0x1D, 0xD0, 0xFC, 0x8F, 0xAA, 0xF4, 0x8F,
...
bool8 S9xMixSamples(uint8 *buffer, int sample_count)
{
    ...
    if (Settings.Mute) {
        ...
        if (spc::resampler->avail() >= (sample_count + spc::lag)) {
            ...
            memset(buffer, (Settings.SixteenBitSound ? 0 : 128),
                   (sample_count << (Settings.SixteenBitSound ? 1 : 0)) >> (Settings.Stereo ? 0 : 1));
            if (spc::lag == 0)
...
static void UpdatePlaybackRate(void)
{
    ...
    double time_ratio = (double)Settings.SoundInputRate * spc::timing_hack_numerator /
                        (Settings.SoundPlaybackRate * spc::timing_hack_denominator);
...
bool8 S9xInitSound(int buffer_ms, int lag_ms)
{
    ...
    spc::sound_enabled = g_snes->S9xOpenSoundDevice();
...
{
    return ((uint8)spc_core->read_port(S9xAPUGetClock(g_snes->scpu->Cycles), port));
}

void S9xAPUWritePort(int port, uint8 byte)
{
    spc_core->write_port(S9xAPUGetClock(g_snes->scpu->Cycles), port, byte);
}
...
void S9xAPUExecute(void)
{
    ...
    spc_core->end_frame(S9xAPUGetClock(g_snes->scpu->Cycles));

    spc::remainder = S9xAPUGetClockRemainder(g_snes->scpu->Cycles);

    S9xAPUSetReferenceTime(g_snes->scpu->Cycles);
}
